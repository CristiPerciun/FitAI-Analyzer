...
### OAuth web (GitHub Pages / hosting statico)

Il login Garmin (CAS) invia il browser all’URL passato come `service=`. In pratica **Garmin spesso non reindirizza** dopo il login se `service` punta a un dominio non accettato (molti casi su `*.github.io`), mentre su `http://localhost:…` può funzionare in sviluppo.
...
**Importante:** `FITAI_WEB_APP_ORIGIN_ALLOWLIST` (e le altre `FITAI_WEB_*`) devono stare nel **`.env` sul Raspberry** nella directory del servizio, caricato da **systemd** (`EnvironmentFile=.../garmin-sync-server/.env` in `garmin-sync.service`), **non** nel `.env` del progetto Flutter sul PC. **Non** basta metterle solo in `/etc/default/garmin-sync-env` se quel file non è referenziato da `garmin-sync.service`. Una riga valida (nessuno spazio intorno a `=`):  
`FITAI_WEB_APP_ORIGIN_ALLOWLIST=https://cristiperciun.github.io/FitAI-Analyzer/`  
Poi: `sudo systemctl restart garmin-sync`. Verifica immediata: apri nel browser `https://myrasberrysyncgar.duckdns.org/` (o il tuo `PUBLIC_SERVER_URL`): nel JSON deve esserci `"oauth_web_allowlist_configured": true`. Se è `false`, il processo non vede la variabile (file sbagliato, typo nel nome, servizio non riavviato).
