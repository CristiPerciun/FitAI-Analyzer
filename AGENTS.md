...
Consulta `.cursor/rules/firestore-collections-structure.mdc` per la struttura aggiornata: `daily_health`, `activities`, `ai_current`.
Consulta `docs/integrations/garmin-ai-flows.md` per il riepilogo dei flussi Garmin e AI.
Consulta `docs/architecture/sync-architecture.md` per sync unificata app + garmin-sync-server (Garmin, Strava, Firestore).
Consulta `docs/architecture/data-architecture.md` per scrittura/lettura dati Firestore.
Consulta `docs/setup/firebase.md` per Firebase (auth, chiavi app/server, deploy regole).
...
- **Collezioni aggiornate**: `daily_health` (Server), `activities` (Server+FitAI), `ai_current` (FitAI) – vedi firestore-collections-structure.mdc
